...
## this seems to now completely work in gtk3
# the spin buttons look dumb...
...
#sval is the starting value of the shim, qval is the starting FWQM value, and name is the name of the shim to adjust
    ...
    blinker.modify_bg(Gtk.StateType.NORMAL,Gdk.color_parse("red"))
...
# get the plot ready:
    ...
    if response == Gtk.ResponseType.REJECT:
        blinker.modify_bg(Gtk.StateType.NORMAL,None)
        return (s0,q0,-1)
...
#    labelHbox.hide()
    ...
    combobox.hide()
    fwqmButton.hide()
    dialog.set_response_sensitive(Gtk.ResponseType.REJECT,False)
...
#    labelHbox.show()
    frame.show()
    dialog.set_response_sensitive(Gtk.ResponseType.REJECT,True)
...
#catch delete events.
    ...
    #dialog = Gtk.Dialog("Shimming Oracle", None, 0, ("Restart",Gtk.ResponseType.REJECT,Gtk.STOCK_OK, Gtk.ResponseType.ACCEPT))
    dialog = Gtk.Dialog("Shimming Oracle", None, 0)
    dialog.add_buttons("Restart",Gtk.ResponseType.REJECT,Gtk.STOCK_OK, Gtk.ResponseType.ACCEPT)

    #    dialog = Gtk.Dialog("Shimming Oracle", None, 0, (Gtk.STOCK_OK, Gtk.ResponseType.ACCEPT))
    dialog.set_transient_for(plotWin.window)
    position = dialog.get_position()

    
    prelabel = Gtk.Label(label = "Enter values of the X1, Z1, and Y1 gradient\ncurrent dials (in that order) and hit OK")
...
#    dialog.vbox.pack_start(labelHbox)
    valuesHbox = Gtk.HBox()
    frameVbox.pack_start(valuesHbox,True,True,0)
#    dialog.vbox.pack_start(valuesHbox)
    labelX = Gtk.Label(label = "X1")
    labelZ = Gtk.Label(label = "Z1")
    labelY = Gtk.Label(label = "Y1")
    labelHbox.pack_start(labelX,True,True,0)
    labelHbox.pack_start(labelZ,True,True,0)
    labelHbox.pack_start(labelY,True,True,0)
    valueX = Gtk.Label(label = "5.00")
    valueZ = Gtk.Label(label = "5.00")
    valueY = Gtk.Label(label = "5.00")
    ...
    label = Gtk.Label(label = "blank")
    dialog.vbox.pack_start(label,True,True,0)

    comboboxHbox = Gtk.HBox()
    blabel = Gtk.Label(label = "")
    ...
    #dialog.vbox.pack_start(combobox,expand=False)
...
#    dialog.vbox.pack_start(shimHbox)
    ...
    shimAdjust1 = Gtk.Adjustment(value = 5.00, lower = 0, upper = 10.0, step_increment = 0.01)
#    shimButton1 = Gtk.SpinButton(shimAdjust1, 0.1, 2) #args are climb_rate and digits
    shimButton1 = Gtk.SpinButton()
    shimButton1.set_adjustment(shimAdjust1)
    shimButton1.set_digits(2)
    shimButton1.set_increments(0.1,1)
    shimButton1.set_numeric(True)
    shimHbox.pack_start(shimButton1,True,True,0)

    shimAdjust2 = Gtk.Adjustment(value = 5.00, lower = 0, upper = 10.0, step_increment = 0.01)
#    shimButton2 = Gtk.SpinButton(shimAdjust2, 0.1, 2) #args are climb_rate and digits
    shimButton2 = Gtk.SpinButton()
    shimButton2.set_adjustment(shimAdjust2)
    shimButton2.set_digits(2)
    shimButton2.set_increments(0.1,1)
    shimButton2.set_numeric(True)
    shimHbox.pack_start(shimButton2,True,True,0)

    shimAdjust3 = Gtk.Adjustment(value = 5.00, lower = 0, upper = 10.0, step_increment = 0.01)
#    shimButton3 = Gtk.SpinButton(shimAdjust3, 0.1, 2) #args are climb_rate and digits
    shimButton3 = Gtk.SpinButton()
    shimButton3.set_adjustment(shimAdjust3)
    shimButton3.set_digits(2)
    shimButton3.set_increments(0.1,1)
    shimButton3.set_numeric(True)
...
#    label.hide()
    ...
    fwqmAdjust = Gtk.Adjustment(value = 5.00, lower = 0, upper = 99.0, step_increment = 0.01)
#    fwqmButton = Gtk.SpinButton(fwqmAdjust, 0.1, 2) #args are climb_rate and digits
    fwqmButton = Gtk.SpinButton() #args are climb_rate and digits
    fwqmButton.set_adjustment(fwqmAdjust)
    fwqmButton.set_digits(2)
    fwqmButton.set_increments(0.1,1)
    fwqmButton.set_numeric(True)
#    dialog.vbox.pack_start(fwqmButton)
#    shimHbox.pack_start(fwqmButton)
...
#    dialog.set_gravity(Gdk.GRAVITY_SOUTH_EAST)
    w,h = dialog.get_size()
    """
    ww = Gdk.Screen.width()
    hh = Gdk.Screen.height()
