...
# sometimes gtk3 blocks during measurement sweep...Can fix with blitting?
...
#               print ' streaming, got: ', self.main.streamWin.tyvals.size, ' points'
            ...
            #then plot it, and we're done.
                GLib.idle_add(self.main.streamWin.updatePlot)
            else:
#                print 'got no data, setting idle function to unpress live view'
                GLib.idle_add(self.unpressLiveView)
                return False
        return True
...
class getDataThread(Thread):
    ...
    def run (self):
        ...
        self.main.killPlay()
        self.doneFlag = True
        GLib.idle_add(self.null) #unblock main_iteration
        return
...
#        toolbar = NavigationToolbar2(self.canvas, self.window)
        ...
        self.saveButton.connect_data("clicked", self.save)
...
#then relabel the xvals so that the transition appears at 0.
    ...
    def save(self, dummy):
        ...
        fileChooser = Gtk.FileChooserDialog(title = "Save file...", parent=None, action=Gtk.FileChooserAction.SAVE)
        fileChooser.add_buttons("Cancel", Gtk.ResponseType.CANCEL, "Save", Gtk.ResponseType.OK )
        ...
        if response == Gtk.ResponseType.CANCEL:
            ...
            try:    #check if fileName exists
                ...
                label = Gtk.Label(label="Duplicate file found! Overwrite?")
...
class topWin:
    ...
    def __init__(self):
        ...
        window = Gtk.Window(type = Gtk.WindowType.TOPLEVEL)    #creates a new window
        try:
...
#        window.set_size_request(400, 300)
        ...
        radButton = Gtk.RadioButton(label = "Off")
        radButton.connect("toggled", self.radButtonChanged, "off")
        rightVbox.pack_start(radButton, True, True, 0)
        self.rad1 = radButton

        radButton = Gtk.RadioButton.new_from_widget(self.rad1)
        radButton.set_label("Function Generator")
        radButton.connect("toggled", self.radButtonChanged, "funcGen")
        rightVbox.pack_start(radButton, True, True, 0)
        self.rad2 = radButton

        radButton = Gtk.RadioButton.new_from_widget(self.rad1)
        radButton.set_label("Measure Using Frequency Sweep")
        radButton.connect("toggled", self.radButtonChanged, "freqSweep")
        rightVbox.pack_start(radButton, True, True, 0)
        self.rad3 = radButton

        radButton = Gtk.RadioButton.new_from_widget(self.rad1)
        radButton.set_label("Measure Using White Noise")
        ...
        thbox = Gtk.HBox()
        rightVbox.pack_start(thbox, True, True, 0)
        label = Gtk.Label(label="Input Signal Source:")
        thbox.pack_start(label, True, True, 0)
        ...
        freqStepView = Gtk.Label(label="Number of Frequency Steps")
        self.freqStepLabel = freqStepView
        self.freqStepAdjustment = Gtk.Adjustment(value = 40, lower = 2, upper = 200, step_increment = 1)
        self.freqStepButton = Gtk.SpinButton()
        self.freqStepButton.set_adjustment(self.freqStepAdjustment)
        self.freqStepButton.set_digits(0)
        self.freqStepButton.set_numeric(True)
        ...
        goButton.connect_data("clicked", self.go)
        ...
        startFreqView = Gtk.Label(label="Start Frequency")
        self.startFreqAdjust = Gtk.Adjustment(value = 200, lower = 1, upper = 9000, step_increment = 100)
        startFreqSpin = Gtk.SpinButton()
        startFreqSpin.set_numeric(True)
        startFreqSpin.set_adjustment(self.startFreqAdjust)
        startFreqSpin.show()

        endFreqView = Gtk.Label(label="End Frequency")
        self.endFreqAdjust = Gtk.Adjustment(value = 4000, lower = 200, upper = 9000, step_increment = 1000)
        endFreqSpin = Gtk.SpinButton()
        endFreqSpin.set_numeric(True)
        endFreqSpin.set_adjustment(self.endFreqAdjust)

        startVal = startFreqSpin.get_value_as_int()
        endVal = endFreqSpin.get_value_as_int()
        stepVal = self.freqStepButton.get_value_as_int()

        freqLabel = Gtk.Label(label="Frequency")
        self.freqAdjust = Gtk.Adjustment(value = startVal, lower = startVal, upper = endVal, step_increment = 10, page_increment = 100) #start,lower,upper,step,page
        ...
        freqSpinButton = Gtk.SpinButton()
        freqSpinButton.set_adjustment(self.freqAdjust)
        freqSpinButton.set_increments(10, 100)
        freqSpinButton.set_digits(1)
        freqSlider = Gtk.HScale()
        freqSlider.set_adjustment(self.freqAdjust)
        freqSlider.set_digits(1)

        self.liveView = Gtk.ToggleButton(label = "Live View")

        self.ampAdjust = Gtk.Adjustment(value = 50, lower = 1, upper = 100, step_increment = 5)
        self.ampSpin = Gtk.SpinButton()
        self.ampSpin.set_adjustment(self.ampAdjust)
        self.ampSpin.set_numeric(True)
        ampLabel = Gtk.Label(label="Amplitude")
        self.amp = self.ampAdjust.get_value()
        
        frame1 = Gtk.Frame()
        
        leftVbox.pack_start(frame1, True, True, 0)
        frameVbox = Gtk.VBox(homogeneous = True)
...
#                   popup_msg("Error running program: "+retVal)
                GLib.idle_add(popupIdleWrap, "Error running program: "+retVal, base.window)
                return None 
...
#               print 'call for record is: ',call
            try:
                ...
                GLib.idle_add(popupIdleWrap, "Problem starting sox to record", base.window)
                return None
            if rval != 0:
                GLib.idle_add(popupIdleWrap, "Problem starting sox to record", base.window)
                return None
...
#on windows, last 20 ms is no good?
            if data.size > 1600:
                data = data[0:-1600]
#baseline correct / subtract off average:                
