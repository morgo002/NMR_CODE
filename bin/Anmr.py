...
# migrate to gtk3 and python 3:
# only thing missing here is gtk agg support in python 3. Works ~ok with gtkcairo backend...
# but has limited number of points (18980 ?) should maybe try to workaround this?
# eg, just before each set_data, check if we have more than 18980 points, and if so, decimate them.
# maybe try scipy resample or scipy decimate
# see: https://github.com/matplotlib/matplotlib/issues/1227
#Jan 2015: instead of pycairo, use cairocffi - solves this - now fully functional in python3/gtk3!
#Hmm - maybe doesn't need cairocffi? is ok without it.

# troubles: ft slow with spectrum points large and not a power of 2. 
# numpy's fault, not ours
...
#with cairocffi rather than pycairo, this works:
    from matplotlib.backends.backend_gtk3agg import FigureCanvasGTK3Agg as FigureCanvas
#    from matplotlib.backends.backend_gtk3cairo import FigureCanvasGTK3Cairo as FigureCanvas
#with pillow 2.0 rather than bare Imaging, this works:
    from PIL import Image
...
class runProgramThread(Thread):
    ...
    def run (self):
        ...
        self.doneFlag =True
        GLib.idle_add(self.null) #unblock the main_iteration
        return
...
#           self.win.set_keep_above(True)
        ...
        vbox = Gtk.VBox(homogeneous=False, spacing=0)           #and a vertical box
        self.win.add(vbox)

        self.totalOps = numOperations
        self.currentOp = 0

        title = Gtk.Label(label="Process: "+name)     #label with name of the process
        vbox.pack_start(title, True, True, 0)
        self.progBar = Gtk.ProgressBar()    #make a progress bar
        self.progBar.set_orientation(Gtk.Orientation.HORIZONTAL)
        ...
        self.abortButton = Gtk.Button(label="Abort")
        self.abortButton.connect("clicked", self.abort)
        ...
        monitor = Gdk.Display.get_monitor_at_window(display,base.window.get_window())
        geometry = monitor.get_geometry()
        print("geom:",geometry.width,geometry.height)
        print("position",geometry.x,geometry.y)
        work = monitor.get_workarea()
        print("work area:",work.width,work.height)
        ww = geometry.width+geometry.x
        ...
        for m in range(n_monitors):
            ...
            geometry = monitor.get_geometry()
            print("monitor %d: %d x %d %d %d" % (m, geometry.width, geometry.height, geometry.x, geometry.y))
        """
        #ww = Gdk.Screen.width()
        self.win.move(ww-200, 20)
    ...
    def abort(self, flag=None):          #this function is called when the abort button is clicked
        self.abortFlag = True
        self.abortButton.set_label("Aborting, please hold...")
        self.abortButton.modify_bg(Gtk.StateType.NORMAL, Gdk.color_parse("red"))
        setAbortFlag()
...
#in full echo mode, throw away the phase? Maybe not actually...
            #               self.phaselabel = Gtk.Label(label=str(0))
            #               self.appliedPhase = 0
...
#                       print ' calling fft with points: ',filledData.size
    ...
    def phasing(self, filler = None, event = None):
        ...
        leftIndex = int(lims[0] / self.step)
        rightIndex = int(lims[1] / self.step)+1
    ...
    def procParamChanged(self,filler=None, dummy=None):
...
#               print leftEdge,rightEdge
    ...
    def measurePoints(self, ToolBar):
        ...
        self.label = Gtk.Label(label="Please click on the left limit of the peak")
...
#        if VER == 3:
#            popup_msg("No imaging library (PIL) in Python 3 yet!!",self.window)
#            return

        #this function reconstructs an image based on the data collected
        if not self.limsSet:
            dialog = Gtk.Dialog("Error", self.window)
            label = Gtk.Label(label="Please use 'Pick Peak Limits' to set bounds for reconstruction.")
            dialog.vbox.pack_start(label, True, True, 0)
            label.show()
...
# TASK 1: find center of projections
        ...
        ll = numpy.zeros(self.numDataFiles,dtype=numpy.int32)
        rr = numpy.zeros(self.numDataFiles,dtype=numpy.int32)
...
# TASK 2: measure width of projections to prepare filter
        ...
        summ = numpy.zeros(int(right)-int(left))
...
#ok, now had Hahn filter
        jc = int(15 * (width) / float(newWidth) )
...
#           os.system(call)
    ...
    def blink(self, killCommand = None, source_id = None):
        if not self.measuring or killCommand == "Kill":
            ...
            self.ebBlinker.modify_bg(Gtk.StateType.NORMAL, None)
            ...
            if self.coloured:
                self.ebBlinker.modify_bg(Gtk.StateType.NORMAL, Gdk.color_parse("yellow"))
                self.coloured = False
            else:
                if self.waiting:
                    self.ebBlinker.modify_bg(Gtk.StateType.NORMAL, Gdk.color_parse("blue"))
                else:
                    self.ebBlinker.modify_bg(Gtk.StateType.NORMAL, Gdk.color_parse("red"))
                self.coloured = True
            return True
    ...
    def plusMinus(self, value):
        ...
        if value == self.plusButton and (self.index < self.numDataFiles-1):
            self.index += 1
            self.minusButton.set_sensitive(True)
            if self.index == self.numDataFiles-1:
                self.plusButton.set_sensitive(False)
        elif value == self.minusButton and (self.index > 0):
            self.index -= 1
...
#            self.phaseAdjust.destroy()
    ...
    def phaseDone(self, button=None, filler=None):
...
#               print 'phaseDone, leaving'
    ...
    def phaseClicked(self, filler=None):
        ...
        if not self.phaseOpen:
            dialog = Gtk.Dialog("Phase Adjust", self.window, destroy_with_parent=True)
            dialog.set_default_size(300,100)
            position = dialog.get_position()
            dialog.move(position[0], position[1] + 200)
            label = Gtk.Label(label="Phase")
            label.show()
            dialog.vbox.pack_start(label, True, True, 0)
            self.phaseAdjust = Gtk.Adjustment(value=self.appliedPhase,lower=0, upper=360, step_increment=1, page_increment=1)
            phaseSlider = Gtk.HScale()
            phaseSlider.set_adjustment(self.phaseAdjust)
            phaseSlider.connect_data("value-changed", self.phasing, None )
            phaseSlider.show()
            ...
            cancelButton.connect_data("clicked", self.phaseDone, "Cancel", connect_flags=GObject.ConnectFlags.SWAPPED)
            cancelButton.show()
            dialog.connect("delete_event",self.phaseDelete)
            applyButton = Gtk.Button(label="Apply")
            applyButton.connect_data("clicked", self.phaseDone, "Apply", connect_flags=GObject.ConnectFlags.SWAPPED)
            applyButton.show()
...
# this will copy the raw data into the selected directory and and also write the FT'd spectra.
    def saveData(self, filler=None):
        ...
        fileChooser = Gtk.FileChooserDialog(title = "Save As...", parent=self.window, action=Gtk.FileChooserAction.CREATE_FOLDER)
        fileChooser.add_buttons("Cancel",Gtk.ResponseType.CANCEL, "Open", Gtk.ResponseType.OK)
        ...
        #otherwise ask for overwrite confirmation
                ...
                dialog.vbox.pack_start(label, True, True, 0)
                dialog.add_buttons("Cancel", Gtk.ResponseType.NO, "Overwrite", Gtk.ResponseType.YES)
                dialog.show_all()
...
#        print ('key %s (%d) pressed' % (keyname,event.keyval))
        if event.state & Gdk.ModifierType.CONTROL_MASK and keyname == 'w':
...
#        self.canvas = canvas
        ...
        vboxPrime = Gtk.VBox(homogeneous=False, spacing=8)
        hbox.pack_start(vboxPrime, False, True, 0)
        fvalue = Gtk.Label(label=self.acqName)

        vboxPrime.pack_start(fvalue, True, True, 0)
        fvalue.set_ellipsize(Pango.EllipsizeMode.START)
        fvalue.set_max_width_chars(20) #without this, it won't ellipsize, will just expand
...
#args[0] is the names, then min, max value, units
        ...
        if self.string == 'Array':
            hboxtemp = Gtk.HBox()
            label = Gtk.Label(label="Arrayed Variable:")
            label.show()
            hboxtemp.pack_start(label, True, True, 0)
            label = Gtk.Label(label=self.arrVar.strip())
            label.show()
            hboxtemp.pack_start(label, True, True, 0)
            vboxPrime.pack_start(hboxtemp, True, True, 0)
            hboxtemp = Gtk.HBox()
            label = Gtk.Label(label="Current Value:")
            label.show()
            hboxtemp.pack_start(label, True, True, 0)
            self.varValLabel = Gtk.Label(label=str(self.arrVal[self.index]))
            self.varValLabel.show()
        ...
        varHbox = Gtk.HBox(homogeneous=False, spacing=0)
        vboxPrime.pack_start(varHbox, True, True, 0)
        vbox1 = Gtk.VBox(homogeneous=True, spacing=2)
        vbox2 = Gtk.VBox(homogeneous=True, spacing=2)
        ...
        for i in range(len (varNames) ):
            ...
            name = Gtk.Label(label= toInsert +" :")
            value = Gtk.Label(label= varVals[i] )
        ...
        if self.string == "Image":
            ...
            name = Gtk.Label(label="Number of Projections: ")

            if not self.measuring:
                value = Gtk.Label(label= str(self.numDataFiles -1)+'+1')
            else:
                value = Gtk.Label(label="1")
            vbox1.pack_start(name, True, True, 0)
            vbox2.pack_start(value, True, True, 0)
            self.numProjsLabel = value

        name = Gtk.Label(label="Total time domain points: ")
        value = Gtk.Label(label= str(self.rawData.shape[-1]))
        vbox1.pack_start(name, True, True, 0)
        vbox2.pack_start(value, True, True, 0)

        hboxtemp = Gtk.HBox(homogeneous=False)
        vboxPrime.pack_start(hboxtemp, True, True, 0)
        name = Gtk.Label(label="Scans: ")
        self.scanLabel = Gtk.Label(label=str(self.numScans[0]))
        vbox1.pack_start(name, True, True, 0)
        vbox2.pack_start(self.scanLabel, True, True, 0)
        varHbox.show_all()

        indexLabel = Gtk.Label(label="Index: ")
        indexLabel.show()
        self.indexVal = Gtk.Label(label=str(self.index+1) +'/'+str(self.numDataFiles))
        self.indexVal.show()
        ...
        self.minusButton = Gtk.Button(label="-")
        self.minusButton.set_property('height-request', 50)
        self.minusButton.show()
        self.minusButton.connect_object("clicked", self.plusMinus, self.minusButton)

        self.plusButton = Gtk.Button(label="+")
        self.plusButton.set_property('height-request', 50)
        self.plusButton.show()
        self.plusButton.connect_object("clicked", self.plusMinus, self.plusButton)
        self.plusButton.set_sensitive(False)
        self.minusButton.set_sensitive(False)
        ...
        self.phaseButton = Gtk.Button(label="Phase")
        vboxTemp.pack_start(self.phaseButton, True, True, 0)
        self.phaseButton.connect("clicked", self.phaseClicked)
        self.phaselabel = Gtk.Label(label=str(self.appliedPhase))
        vboxTemp.pack_start(self.phaselabel, True, True, 0)
        label = Gtk.Label(label="Broadening")
        vboxTemp.pack_start(label, True, True, 0)
        self.lbAdjust = Gtk.Adjustment(value = base.broadening, lower = 0, upper = 50, step_increment = 0.1)
        self.lbspin_button = Gtk.SpinButton() #args are climb_rate and digits
        self.lbspin_button.set_adjustment(self.lbAdjust)
        self.lbspin_button.set_increments(0.1,1)
        self.lbspin_button.set_digits(1)
        self.lbspin_button.set_numeric(True)

        vboxTemp.pack_start(self.lbspin_button, True, True, 0)

        vboxTemp = Gtk.VBox()
        hboxPrime.pack_start(vboxTemp, True, True, 0)
        label = Gtk.Label(label="Left Shift")
        vboxTemp.pack_start(label, True, True, 0)
        self.leftShift = Gtk.Adjustment(value = base.leftShift, lower=-100000, upper=100000, step_increment=1)
        self.shiftSpinButton = Gtk.SpinButton()
        self.shiftSpinButton.set_adjustment(self.leftShift)
        self.shiftSpinButton.set_numeric(True)
        vboxTemp.pack_start(self.shiftSpinButton, True, True, 0)

        label = Gtk.Label(label="Spectrum Points")
        vboxTemp.pack_start(label, True, True, 0)

        fnpts = base.freqDomExpand
        self.freqDomExpand = Gtk.Adjustment(value=fnpts, lower=64, upper=131072, step_increment = 1)
        self.freqDomExpandButton = Gtk.SpinButton()
        self.freqDomExpandButton.set_adjustment(self.freqDomExpand)
        self.freqDomExpandButton.set_increments(64,64)
        self.freqDomExpandButton.set_numeric(True)
        ...
        self.echoCheck = Gtk.CheckButton(label="Full Echo")
        multCheckHbox = Gtk.HBox()
        self.multCheck = Gtk.CheckButton(label="Multiple Echoes")
        if base.multCheck:
        ...
        self.multNumAdj = Gtk.Adjustment(value=base.necho,lower=1,upper=64,step_increment=1)
        spinButton = Gtk.SpinButton()
        spinButton.set_adjustment(self.multNumAdj)
        ...
        self.saveDataButton.connect_object("clicked", self.saveData, self.saveDataButton)

        hboxPrime.pack_start(self.saveDataButton, True, True, 0)

        self.saveSpecButton = Gtk.Button(label=" Write\nSpectrum")
        self.saveSpecButton.show()
        self.saveSpecButton.connect_object("clicked",self.saveSpec,self.saveSpecButton)
        hboxPrime.pack_start(self.saveSpecButton, True, True, 0)


        makeImageButton = Gtk.Button(label="Reconstruct\n  Image")
#           makeImageButton.set_property('height-request', 75)
        makeImageButton.show()
        makeImageButton.connect("clicked", self.reconstructImage)
        self.makeImageButton = makeImageButton
        hboxPrime.pack_start(makeImageButton, False, False, 0)

        self.autoAlignCheck = Gtk.CheckButton(label="Auto Align\nProjections")
        hboxPrime.pack_start(self.autoAlignCheck, True, True, 0)
        if base.autoAlign:
            self.autoAlignCheck.set_active(True)

        self.circleCheck = Gtk.CheckButton(label="Circle Correct\nProjections")
        ...
        self.lbspin_button.connect_data("value-changed", self.procParamChanged, "broad", connect_flags=GObject.ConnectFlags.SWAPPED)
        leftHandler = self.shiftSpinButton.connect_data("value-changed", self.procParamChanged, "leftShift", connect_flags=GObject.ConnectFlags.SWAPPED)
        freqDomHandler = self.freqDomExpandButton.connect_data("value-changed", self.procParamChanged, "freqDom", connect_flags=GObject.ConnectFlags.SWAPPED)
        self.multCheck.connect_data("toggled", self.procParamChanged, "multCheck", connect_flags=GObject.ConnectFlags.SWAPPED)
        self.echoCheck.connect_data("toggled", self.procParamChanged, "echoCheck", connect_flags=GObject.ConnectFlags.SWAPPED)
        self.multNumAdj.connect_data("value_changed",self.procParamChanged, "multNum", connect_flags=GObject.ConnectFlags.SWAPPED)
        self.autoAlignCheck.connect_data("toggled",self.procParamChanged,"autoAlignCheck", connect_flags=GObject.ConnectFlags.SWAPPED)
        self.circleCheck.connect_data("toggled",self.procParamChanged,"circleCheck", connect_flags=GObject.ConnectFlags.SWAPPED)
        retValues = []
        ...
        if not self.measuring:
            ...
            vboxPrime.pack_start(hboxtemp, True, True, 0)
            if incomplete == 1:
                name = Gtk.Label(label="INCOMPLETE Date: ")
            else:
                name = Gtk.Label(label="Date: ")
            value = Gtk.Label(label=olddate)
            hboxtemp.pack_start(name, True, True, 0)
            hboxtemp.pack_start(value, True, True, 0)

            hboxtemp = Gtk.HBox()
            vboxPrime.pack_start(hboxtemp, True, True, 0)
            name = Gtk.Label(label="Time: ")
            value = Gtk.Label(label=oldtime)
            hboxtemp.pack_start(name, True, True, 0)
            hboxtemp.pack_start(value, True, True, 0)

        hboxtemp = Gtk.HBox()
        vboxPrime.pack_start(hboxtemp, True, True, 0)
        name = Gtk.Label(label="Program: ")
        value = Gtk.Label(label=oldfileName.strip())
        hboxtemp.pack_start(name, True, True, 0)
        hboxtemp.pack_start(value, True, True, 0)

        if self.measuring:

            self.blinker = Gtk.Label(label="In Progress...")
            ...
            self.waiting = False
            base.measuring = True
            self.source_id = GLib.timeout_add(250, self.blink)   #500 ms, calls blink
...
class progSelect:
    ...
    measuring = False
    vbox = Gtk.VBox(homogeneous = False, spacing = 0)       #this box is the global box for this window
...
#"global" processing parameters
    ...
    def __init__(self):
        ...
        self.window = Gtk.Window(type = Gtk.WindowType.TOPLEVEL)   #creates a new window
...
#        self.window.set_property('allow-grow', True)
        ...
        self.vbox = Gtk.VBox(homogeneous = False, spacing = 1) # false - not homogeneous, and 1 pixel spacing.
...
#rest of the main window
        ...
        hbox = Gtk.HBox(homogeneous = False, spacing = 0)               #add a horizontal box
        ...
        numRuns = Gtk.Label(label="Number of Runs")
        self.comps.append(numRuns)              #just to be able to reference it (1)

        hbox.pack_start(numRuns, True, True, 0)     #pack into the horiz box

        adjustment = Gtk.Adjustment(value = 4, lower = 1, upper = 9001, step_increment = 1)      #making an adjustment object
        spin_button = Gtk.SpinButton()  #using adjustment obj to make spin button obj
        spin_button.set_adjustment(adjustment)
        spin_button.set_increments(1,1)
        ...
        spectraButton = Gtk.Button(label = "Acquire image data")
        #spectraButton.connect_object("clicked", self.acquire, 'Image')
        spectraButton.connect_data("clicked", self.acquire, 'Image')
    ...
    def openData(self,widget,mode):
        # mode is acq or saved
        fileChooser = Gtk.FileChooserDialog(title = "Open folder...", parent=self.window, action=Gtk.FileChooserAction.SELECT_FOLDER)
        fileChooser.add_buttons("Cancel",Gtk.ResponseType.CANCEL, "Open", Gtk.ResponseType.OK)
        if mode == 'acq':
...
#           print ' got : ',response, 'from filechooser, and foldName: ',foldName, 'and name: ',fileChooser.get_filename()
    ...
    def updateProgMenu(self):
        ...
        for i in range(len(self.progListShow)):         #makes an a MenuItem object for each file
            ...
            tempMenuItem = Gtk.MenuItem(label=name)
            progMenuList.append(tempMenuItem)
            tempMenuItem.connect("activate", self.menuItemSelected, name)
            tempMenuItem.show()

        self.progMenuLabel = Gtk.MenuItem(label = "Data Acquisition")
        ...
        self.fileLabel = Gtk.Label(label="\t \t \t ")
    ...
    def changeFold(self, widget, string):
        global DATA_DIR
        fileChooser = Gtk.FileChooserDialog(title = "Select Default Data folder...", parent=self.window, action=Gtk.FileChooserAction.SELECT_FOLDER, buttons=("Cancel",Gtk.ResponseType.CANCEL, "Open", Gtk.ResponseType.OK) )
        fileChooser.set_current_folder(DATA_DIR)
        response = fileChooser.run()
        foldName = fileChooser.get_current_folder()
        name = fileChooser.get_filename()
...
# confirm ?
        if self.data_set_num > -1:
            ...
            label = Gtk.Label(label="Really Quit? All unsaved session data will be lost.")
            dialog.vbox.pack_start(label, True, True, 0)
            dialog.add_buttons("Cancel", Gtk.ResponseType.NO, "Quit", Gtk.ResponseType.YES)
            dialog.show_all()
            response = dialog.run() # -8 is yes, and -9 is no
            dialog.destroy()
            if (response != Gtk.ResponseType.YES ):
...
#               mytime = datetime[11:19]
    ...
    def acquire(self, dummy=None, type=None):
        # should grey it out...
        if self.measuring:
...
# check the arduino out.
        ...
        if type == 'Array':
            ...
            label = Gtk.Label(label="Variable to array:")
            dialog.vbox.pack_start(label, True, True, 0)
            ...
            label = Gtk.Label(label="Number of values:")
            dialog.vbox.pack_start(label, True, True, 0)

#spin button for number of values to take
            adjustment = Gtk.Adjustment(value=2,lower=2, upper=9000, step_increment=1)
            spinButton = Gtk.SpinButton()
...
#okay and abort buttons
            dialog.add_button("Abort", Gtk.ResponseType.CLOSE)
            dialog.add_button("Ok", Gtk.ResponseType.YES)
        ...
        #with the name of the variable at the top
            if self.varUnits == "" or self.varUnits == "''" or self.varUnits == '""':
                varLabel = Gtk.Label(label=varName)
            else:
                varLabel = Gtk.Label(label=varName + ' ('+self.varUnits[index]+')')
        ...
        #use limits and current value.
            ...
            for i in range(numReads):
                tempAdj = Gtk.Adjustment(value=val, lower=self.varMins[index], upper=self.varMaxs[index], step_increment=1)
                tempSpinBut = Gtk.SpinButton()
                tempSpinBut.set_numeric(True)
                tempSpinBut.set_adjustment(tempAdj)
...
#               print varName,varVals
        ...
        #ask user to tell us how many projections.
            ...
            label = Gtk.Label(label="Please input the number of projections you wish to take")
            dialog.vbox.pack_start(label, True, True, 0)
            adjustment = Gtk.Adjustment(value=12, lower=2, upper=9001, step_increment=1)     #making an adjustment object
            spin_button = Gtk.SpinButton()  #using adjustment obj to make spin button obj
            spin_button.set_adjustment(adjustment)
            spin_button.set_increments(1,1)
            spin_button.set_numeric(True)
...
#           print 'Will be doing a total of numReads x numRuns: ',numReads,' x ',numRuns
        ...
        if type == 'Image':
            dialog = Gtk.Dialog("Anmr", base.window,Gtk.DialogFlags.MODAL)
            label = Gtk.Label(label=" ")
            dialog.vbox.pack_start(label, True, True, 0)
            dialog.add_button("Abort", Gtk.ResponseType.CLOSE)      #this is -7
            backButton = dialog.add_button("Back",Gtk.ResponseType.CANCEL)          #this is -6
            dialog.add_button("Ok", Gtk.ResponseType.OK)        #this is -5
            ...
            if type == 'Image': # all this is to deal with being able to go back to previous runs and confirming cancel
                ...
                while (go_on is False):
                    if ( i == numReads-1 ):
                        ...
                        dialog.modify_bg(Gtk.StateType.NORMAL,Gdk.color_parse('yellow'))
                    else:
                        dialog.modify_bg(Gtk.StateType.NORMAL,None)
                    ...
                    while True:
                        response = dialog.run()
                        if response == Gtk.ResponseType.CLOSE or response == Gtk.ResponseType.CANCEL or response == Gtk.ResponseType.OK:
                            break
                    dialog.hide()
                    if set is not None:
                        set.waiting = False
                    ...
                    if response == Gtk.ResponseType.CLOSE:
                        ...
                        nlabel = Gtk.Label(label="Are you sure? \nYou will be unable to continue this acquisition.")
                        confirmDialog.vbox.pack_start(nlabel, True, True, 0)
                        confirmDialog.add_buttons("No", Gtk.ResponseType.NO, "Yes, Abort", Gtk.ResponseType.YES)
                        confirmDialog.show_all()
                        cResponse = confirmDialog.run()
                        confirmDialog.destroy()
                        if cResponse == Gtk.ResponseType.YES:
                            self.cleanup(set,"Cancelled")
                            return
                    elif response == Gtk.ResponseType.CANCEL: #"Back"
                        if i == 0:
...
#for slow cpu's next two lines help to prevent lost read data points
                ...
                while not myThread.doneFlag:
                    Gtk.main_iteration() # run the gui
                ...
                if (j==0 and i==0 and set is None): # its possible we got a back to the beginning. Don't redo this.
                    ...
                    if retData is not None and retData.size != set.rawData[0].size:
                        if warned == 0:
                            ...
                            warned = 1
                        if set.rawData[0].size < retData.size: #make the new data the same length as old data
...
#user chose a pulse program.
        ...
        self.fileLabel = Gtk.Label(label=string)
        ...
        for i in range(len(self.varNames)):             #creation of new objects
            ...
            text = Gtk.Label(label=toInsert)

            self.varEntries.append( [text, 0] )
            hbox.pack_start(text, True, True, 0)

            adjustment = Gtk.Adjustment(value=self.varVals[i],lower=self.varMins[i],upper=self.varMaxs[i], step_increment=1, page_increment=10, page_size=0)
